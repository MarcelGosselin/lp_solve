...
static struct {
  ...
  { "ACCURACYERROR", ACCURACYERROR, consttype_solve, ~0, TRUE },
...
/* return = xxlpsolve('get_accuracy', lp) */

static void impl_get_accuracy(structlpsolve *lpsolve)
{
        Double  *dpr;

        Check_nrhs(lpsolve, 1);
        dpr = CreateDoubleMatrix(&lpsolve->lpsolvecaller, 1, 1, 0);
        *dpr = get_accuracy(lpsolve->lp);
        SetDoubleMatrix(&lpsolve->lpsolvecaller, dpr, 1, 1, 0, TRUE);
}


/* return = xxlpsolve('get_break_numeric_accuracy', lp) */

static void impl_get_break_numeric_accuracy(structlpsolve *lpsolve)
{
        Double  *dpr;

        Check_nrhs(lpsolve, 1);
        dpr = CreateDoubleMatrix(&lpsolve->lpsolvecaller, 1, 1, 0);
        *dpr = get_break_numeric_accuracy(lpsolve->lp);
        SetDoubleMatrix(&lpsolve->lpsolvecaller, dpr, 1, 1, 0, TRUE);
}
...
static struct {
  ...
  { "get_accuracy", impl_get_accuracy, TRUE },
  { "get_break_numeric_accuracy", impl_get_break_numeric_accuracy, TRUE },
